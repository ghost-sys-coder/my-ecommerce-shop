...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (err === "Wrong password!") {
      ...
      err ===
      "Email is not verified! Check your mailbox for a verification link!"
    ) {
      return res
        .status(401)
        .json({
    ...
    return res
      .status(500)
      .json({ error: "Login failed! Check your credentials!" });
  }
};
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return res.status(401).json({ message: "Your email is not registered!" });

    user.resetCode = crypto.randomInt(0, 1000000).toString().padStart(6, "0");
    user.resetCodeExpiration = new Date().setMinutes(
...
const resendEmailVerificationToken = async (req, res) => {
  try {
    ...
    const isVerified = await User.findOne({ email, verified: true });

    if (isVerified) {
      return res.status(400).json({ message: "This email is verified!" });
    }
    ...
    return res
      .status(200)
      .json("Your email verification token has been sent to your email");
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: error.message });
  }
